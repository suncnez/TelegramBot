...
var gUsefulActivities = Activities{
	{"Go", "Programming (2 hours)", 5},
	{"Gym", "Training (2 hours)", 3},
	{"Read", "Reading (1 hour)", 2},
	{"Clean", "Cleaning the room", 1},
}

var gRewards = Activities{
	{"YT", "Watch letsplay on youtube", 3},
}
...
func printIntro(update *tgbotapi.Update) {
	...
	printSystemMessageWithDelay(9, "There are numerous beneficial actions that, by performing regularly, we improve the quality of our life. However, often it's more fun, easier, or tastier to do something harmful. Isn't that so?")
	printSystemMessageWithDelay(9, "With greater likelihood, we'll prefer to get lost in YouTube Shorts instead of an English lesson, buy another croissant in the coffee shop instead of vegetables, or lie in bed instead of going to gym!.")
	printSystemMessageWithDelay(1, EMOJI_MONK)
	...
	printSystemMessageWithDelay(2, `For example, you spend an hour for physical activities, for which you get 2 coins. After that, you have 2 hours of programming study, for which you get 8 coins. Now you can watch 1 episode of "Interns" and break even. It's that simple!`)
	printSystemMessageWithDelay(9, `Mark completed useful activities to not lose your coins. And don't forget to "purchase" the reward before actually doing it.`)
}
...
func askToPrintIntro() {
	msg := tgbotapi.NewMessage(gChatId, "Get insruction")
...
func showMenu() {
	msg := tgbotapi.NewMessage(gChatId, "Chose:")
	msg.ReplyMarkup = tgbotapi.NewInlineKeyboardMarkup(
...
func showBalance(user *User) {
	msg := fmt.Sprintf("%s, Your wallet is empty...%s \nTrack your first activity!", user.name, EMOJI_DONT_KNOW)
	if coins := user.coins; coins > 0 {
		msg = fmt.Sprintf("%s, My coins: %d %s", user.name, coins, EMOJI_COIN)
	}
...
func showUsefulActivities() {
	showActivities(gUsefulActivities, "Track activity or go back menu:", true)
}

func showRewards() {
	showActivities(gRewards, "Get reward!", false)
}
...
func processUsefulActivity(activity *Activity, user *User) {
	...
	if activity.coins == 0 {
		...
		errorMsg = fmt.Sprintf("Max coins %d %s", MAX_USER_COINS, EMOJI_COIN)
	}
	resultMessage := ""
	if errorMsg != "" {
		resultMessage = fmt.Sprintf("%s, %s %s Your balance full...", user.name, errorMsg, EMOJI_MONK)
	} else {
		user.coins += activity.coins
		resultMessage = fmt.Sprintf("Activity done! reward is %d %s. Balance is: %d", activity.coins, activity.name, activity.coins)
	}
...
func processReward(activity *Activity, user *User) {
	...
	if activity.coins == 0 {
		...
		errorMsg = fmt.Sprintf("Balance: %d. No coins for %s", user.coins, activity.name)
	}
	resultMessage := ""
	if errorMsg != "" {
		resultMessage = fmt.Sprintf("Balance without change")
	} else {
		user.coins -= activity.coins
		resultMessage = fmt.Sprintf("Reward is paid! Balance %d", user.coins)
	}
	gBot.Send(tgbotapi.NewMessage(gChatId, resultMessage))
}
...
func updateProcessing(update *tgbotapi.Update) {
	...
	if !found {
		if user, found = storeUserFromUpdate(update); !found {
			gBot.Send(tgbotapi.NewMessage(gChatId, "User not found"))
			return
		}
	}
	...
	default:
		...
		log.Printf("[%T] ERROR: %s", time.Now(), choiceCode)
		msg := fmt.Sprintf("%s, code unknown '%s' %s Get contact with creator", user.name, choiceCode, EMOJI_MONK)
		gBot.Send(tgbotapi.NewMessage(gChatId, msg))
	}
}
